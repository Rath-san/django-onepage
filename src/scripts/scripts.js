...
(() => {
    ...
    window.addEventListener("load", () => {
        ...
        const sections = [
            ...Array.from(document.querySelectorAll("section:not(.header, #silicon_bar)")),
    ...
    const makeSlider = (id, options = {}, touch = true) => {
        ...
        // $(id).carousel("pause");
        touch && carouselSwiping(slider[0]);

        return slider;
    };

    const initSlider = () => {
        const ids = ["#carouselFlat", "#carouselFlat2"];
        ids.forEach((id) => {
            makeSlider(id);

            const tagsSliderId = id + "Tags";
            makeSlider(tagsSliderId, {}, false);

            const indicators = $(`${id}Indicators > li`);

            if (indicators) {
                const carousel = $(id);
                const carouselTags = $(tagsSliderId);

                carousel.on("slide.bs.carousel", (e) => {
                    carouselTags.carousel(e.to);
    ...
    Array.from(document.querySelectorAll(".collapse__prevs__btn")).forEach((btn, i) => {
